@php
    ...
    $collection = $collection ?? 'failure_reports';
    $center = $center ?? false; 
    $sinAdjuntos = $sinAdjuntos ?? 'Sin adjuntos'; 


    $mediaItems = $getRecord()->getMedia($collection);

$galleryId = class_basename($getRecord()) . '-' . $getRecord()->getKey();
@endphp

@if ($mediaItems->isNotEmpty())
    <div class="{{ $center ? 'flex flex-wrap justify-center gap-3' : 'grid grid-cols-2 md:grid-cols-3 gap-3' }}">
        @foreach ($mediaItems as $item)
            ...
            @if ($isImage)
                <a href="{{ $item->getUrl() }}" class="glightbox" data-gallery="{{ $galleryId }}">
                    <img
                        src="{{ $item->hasGeneratedConversion('thumb') ? $item->getUrl('thumb') : $item->getUrl() }}"
                        alt="{{ $item->name }}"
            ...
            @elseif ($isPdf)
                <a href="{{ $item->getUrl() }}" class="glightbox"
                   ...
                   data-width="100%" data-height="90vh">
                    <div class="rounded-xl w-full h-32 bg-gray-100 border flex items-center justify-center dark:bg-gray-800 dark:border-gray-700">
                        ...
                        <span class="ml-2 text-sm font-medium truncate">{{ Str::limit($item->name, 24) }}</span>
                    </div>
                </a>

            @else
                <a href="{{ $item->getUrl() }}" target="_blank" class="block">
                    <div class="rounded-xl w-full h-32 bg-gray-50 border flex items-center justify-center">
                        <span class="text-sm">Archivo: {{ Str::limit($item->file_name, 24) }}</span>
                    </div>
                </a>
            @endif
        @endforeach
...
@else
    <x-filament::section.description>{{ $sinAdjuntos }}</x-filament::section.description>
@endif
